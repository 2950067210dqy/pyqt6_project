...
from ui.customize_ui.tab.tab2_tab4 import Ui_tab_4_frame
from ui.tab7 import Ui_tab7_frame
from util.number_util import number_util


class Tab2_tab4(ThemedWidget):
    # 更新端口选择和鼠笼选择
    ...
    def _init_customize_ui(self):
        ...
        try:
            ...
            logger.error(f"tab_tab4_index图表创建错误：{e}")
        pass
        # 添加最新数据表格table
        self.detaildata_layout: QVBoxLayout = self.findChild(QVBoxLayout, "detaildata_layout")
        # charts!
        try:
            self.detaildata_table = TableWidgetPaging(type=self.type, data_type='monitor_data',
                                                      mouse_cage_number=global_setting.get_setting(
                                                          "tab2_select_mouse_cage"),
                                                      parent=self.detaildata_layout)
        except Exception as e:
            logger.error(f"tab_tab4_index table创建错误：{e}")
        pass
