...
class VideoPlayer(QObject):
    def __init__(self, parent_frame: QWidget, parent_layout: QVBoxLayout, open_video_btn, start_video_btn,
                 stop_video_btn, plainTextEdit, video_slider, video_slider_text):
        super().__init__()
    ...
    def open_file(self):
        ...
        try:
            ...
            open_path = Path.joinpath(current_directory,
                                      ...
                                      global_setting.get_setting("smart_device_server_config")['Storage']['video_path'])
            open_path.mkdir(parents=True, exist_ok=True)
            file_path, _ = QFileDialog.getOpenFileName(self.parent_frame, "打开视频文件", open_path.as_posix(),
                                                       "视频文件 (*.mp4 *.avi *.mkv)")
            if file_path:
