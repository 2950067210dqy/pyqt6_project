...
from smart_device.server.image_process import report_writing
...
class Data_thread(QThread):
    ...
    def __init__(self, update_status_main_signal):
        ...
        self.data = []
        self.data_save = report_writing(file_path=global_setting.get_setting('smart_device_server_config')['Storage'][
                                                      'fold_path'] + f"/{global_setting.get_setting('smart_device_server_config')['Image_Process']['report_file_name']}")
        pass
    ...
    def run(self):
        while True:
            ...
            time.sleep(float(global_setting.get_setting('smart_device_server_config')['Image_Process'][
                                 'delay']))
            # time.sleep(1)
        pass
...
class BarChartApp(ThemedWidget):
    ...
    def _set_data_set(self):
        ...
        extenal_list_data = self.extend_and_return_new_lists_insert_0([int(i) for i in list(self.fl_data.values())],
                                                                      "FL",
                                                                      [int(i) for i in list(self.yl_data.values())],
                                                                      "YL")
    ...
    def _set_y_axis(self):
        # 设置 Y 轴
        # 短的数据项后边补充0
        extenal_list_data = self.extend_and_return_new_lists_insert_elem(list(self.fl_data.keys()), "FL",
                                                                         list(self.yl_data.keys()), "YL")
        keys = []
