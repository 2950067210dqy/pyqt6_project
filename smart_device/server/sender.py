...
class Sender(Thread):
    ...
    def client_init(self):
        ...
        while retry_count < self.max_retries:
            try:
                ...
                logger.info(
                    f"Sender({retry_count + 1}/{self.max_retries}) {self.uid} connected {self.host}:{self.port} successfully")
                return True
    ...
    def run(self):
        ...
        while (self.running):
            self.send_handle()
            time.sleep(float(global_setting.get_setting("smart_device_server_config")[f'Sender_{self.type}']['delay']))
            pass
        pass
    ...
    def send_handle(self):
        ...
        if not self.init_state:
            ...
            if not self.init_state:
                return
        try:
            if self.img_dir is None or self.img_dir == "":
                return
            self.send_image()
    ...
    def send_image(self):
        ...
        try:
            ...
            logger.error(
                f"Error sender{self.uid} encrypted_data——image to server: {e} | trace stack:{traceback.print_exc()}")
            self.client_socket.shutdown(socket.SHUT_WR)  # 重要：半关闭发送端
            self.init_state = False
            if self.client_socket is not None:
                self.client_socket.close()
