...
from smart_device.ui.custom_ui.BarChart import BarChartApp
from smart_device.ui.custom_ui.VideoPlayer import VideoPlayer
from smart_device.ui.tab7 import Ui_tab7_frame
...
class Status_thread(QThread):
    ...
    def reverse_lines_efficient(self, input_string):
        ...
        if length > int(global_setting.get_setting('smart_device_configer')['Status']['max_line']):
            return '\n'.join(
                output_lines[
                1:int(global_setting.get_setting('smart_device_configer')['Status']['max_line'])])  # 跳过第一个空元素
        else:
    ...
    def run(self):
        while True:
            # 实时获取当前status
            filename = "./smart_device/log/report_smart_device/report_" + time.strftime("%Y-%m-%d",
                                                                                        time.localtime()) + ".log"
            str = ""
...
class Tab_7(ThemedWidget):
    ...
    def init_videos(self):
        ...
        self.video_component = VideoPlayer(parent_frame=self.frame, parent_layout=video_layout,
                                           open_video_btn=open_video_btn,
                                           start_video_btn=start_video_btn, stop_video_btn=stop_video_btn,
    ...
    def openFL_Folder(self):
        ...
        current_directory = Path.cwd()
        open_direct = Path.joinpath(current_directory,
                                    global_setting.get_setting("smart_device_server_config")['Storage']['fold_path'],
                                    "FL_" + global_setting.get_setting("smart_device_server_config")['Image_Process'][
                                        'fold_suffix'])
        open_direct.mkdir(parents=True, exist_ok=True)
        os.startfile(open_direct)  # 替换为你要打开的文件夹路径
        pass
    ...
    def openReport_Folder(self):
        ...
        open_direct = Path.joinpath(current_directory,
                                    global_setting.get_setting("smart_device_server_config")['Storage']['fold_path'],
                                    global_setting.get_setting("smart_device_server_config")['Image_Process'][
                                        'report_file_name'])
        # open_direct.mkdir(parents=True, exist_ok=True)
        os.startfile(open_direct)  # 替换为你要打开的文件夹路径
        pass
    ...
    def openYL_Folder(self):
        ...
        open_direct = Path.joinpath(current_directory,
                                    global_setting.get_setting("smart_device_server_config")['Storage']['fold_path'],
                                    "YL_" + global_setting.get_setting("smart_device_server_config")['Image_Process'][
                                        'fold_suffix'])
        open_direct.mkdir(parents=True, exist_ok=True)
        os.startfile(open_direct)  # 替换为你要打开的文件夹路径
        pass
