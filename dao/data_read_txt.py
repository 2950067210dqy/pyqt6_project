...
class data_read_txt():
    ...
    def read_range_datas_seq_from_times_index_start(self, times: datetime = datetime.now(), data_origin_port: list = [],
                                                    data_start: list = [], data_func_choose=[True],
                                                    data_nums: list = [], data_step: list = []) -> [[
        txt_data()]]:
        """
        顺序获取多个文件范围数据 从时间开始读取
        ...
        :param data_func_choose True选择times位置开始读取数据  False 选择index读取数据
        :param data_step 数据步长 每隔几个数据取数据 如果为负值则从后往前隔着取
        :return: [[txt_data()]]
        """
        if len(self.data_origin_port) == 0 and len(data_origin_port) == 0:
        ...
        for index in range(len(data_origin_port_temp)):
            ...
            if data_func_choose[index]:
                # 选择times方式
                data = self.read_range_data_seq_from_times_start(times=times, data_origin_port=data_origin_port[index],
                                                                 data_start=data_start[index],
                                                                 data_nums=data_nums[index], data_step=data_step[index])
            else:
                # 选择index方式
                data = self.read_range_data_seq_from_index_start(times=times, data_origin_port=data_origin_port[index],
                                                                 data_start=data_start[index],
                                                                 data_nums=data_nums[index], data_step=data_step[index])
                pass
            datas.append(data)
